#!/usr/bin/env bash
# Installs CMS on Ubuntu 22.04/24.04 or Debian 12/13 (amd64 or arm64), or
...
parse_args() {
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      -h|--help) if [ -n "$SELF_DIR" ]; then sed -n '2,56p' "${BASH_SOURCE[0]}"; else say "see https://github.com/$REPO/blob/main/scripts/install.sh"; fi; exit 0 ;;
  ...
  if [ "$ROLE" = main ] && [ -z "$DOMAIN" ] && [ "$LAN" = 0 ]; then
    LAN=1
    [ "$UNINSTALL" = 1 ] || say "No --domain: plain HTTP on this machine's addresses (contest :80, ranking :8080, admin :8081)."
  fi
...
detect() {
  ...
  case "$id:$ver" in
    ubuntu:22.04|ubuntu:24.04|debian:12|debian:13) say "  system: $name" ;;
    ubuntu:*|debian:*) warn "$name is not tested (supported: Ubuntu 22.04/24.04, Debian 12/13); carrying on" ;;
    *) blocker "unsupported system ${name:-unknown}: CMS installs on Ubuntu 22.04/24.04 or Debian 12/13 (Windows and macOS cannot judge; see docs/en/deployment.md, or use Docker Compose on another Linux)" ;;
  esac
...
EOF
  ...
  if [ "$DRY" = 1 ]; then
    pgver=$(find /etc/postgresql -mindepth 1 -maxdepth 1 -printf '%f\n' 2>/dev/null | sort -V | tail -1)
    echo "$conf" | write "/etc/postgresql/${pgver:-VERSION}/main/conf.d/cms.conf" 644 || true
    run createdb -O cms -E UTF8 cms
    return 0
  fi
  pgver=$(find /etc/postgresql -mindepth 1 -maxdepth 1 -printf '%f\n' | sort -V | tail -1)
...
valkey() {
  [ "$ROLE" = main ] || return 0
  step "Valkey"
  local bind="127.0.0.1 -::1" conf dir main svc changed=0
...
EOF
  ...
  if [ -d /etc/valkey ]; then dir=/etc/valkey; main=valkey.conf; svc=valkey-server; else dir=/etc/redis; main=redis.conf; svc=redis-server; fi
  if [ "$DRY" = 1 ]; then
    echo "$conf" | write "$dir/cms.conf" 640 || true
    return 0
  fi
  echo "$conf" | write "$dir/cms.conf" 640 && changed=1
  chgrp "$(stat -c %G "$dir/$main")" "$dir/cms.conf"
  grep -q "^include $dir/cms.conf" "$dir/$main" || { echo "include $dir/cms.conf" >> "$dir/$main"; changed=1; }
  systemctl enable "$svc" >/dev/null 2>&1
  if [ "$changed" = 1 ]; then systemctl restart "$svc"; else systemctl start "$svc"; fi
}
...
summary() {
  ...
  if [ "$DRY" = 1 ]; then
    ...
    else say "Dry run: nothing was changed; run again without --dry-run to install $VER."; fi
    return
  fi
  say "CMS $VER installed."
  if [ "$ROLE" = main ]; then
    if [ "$LAN" = 1 ]; then
      say "  contest:  http://<this machine>/          ranking: http://<this machine>:8080/"
      say "  admin:    http://<this machine>:8081/"
    else
...
uninstall() {
  ...
  if [ "$PURGE" = 1 ]; then
    if command -v psql >/dev/null; then
      run runuser -u postgres -- dropdb --if-exists cms
      run runuser -u postgres -- psql -qc "DROP ROLE IF EXISTS cms"
    fi
    ...
    echo
    say "CMS and its data were removed. Left installed: PostgreSQL, Valkey, the reverse proxy, isolate and the compilers (apt remove them if unused); /etc/caddy/Caddyfile still points at CMS."
  else
    echo
