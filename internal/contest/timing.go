...
func (p Phase) String() string {
	return [...]string{"not_started", "waiting_start", "running", "finished", "analysis"}[p]
}
...
func Compute(r Rules, p Participant, now time.Time) Status {
	begin := r.Start.Add(p.Delay)
	stop := r.Stop.Add(p.Delay) // last instant a per-user contestant may start
	...
	if p.Unrestricted {
		s.CanSubmit = true
		// Unrestricted submissions during analysis stay unofficial.
		s.Official = s.Phase != Analysis
	}
	return s
}
