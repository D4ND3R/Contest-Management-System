...
const setContestBanner = `-- name: SetContestBanner :exec
UPDATE contests SET banner_digest = $1, banner_type = $2, updated_at = now() WHERE id = $3
`

type SetContestBannerParams struct {
...
func (q *Queries) SetContestBanner(ctx context.Context, arg SetContestBannerParams) error {
	_, err := q.db.Exec(ctx, setContestBanner, arg.Digest, arg.MediaType, arg.ID)
	return err
}
